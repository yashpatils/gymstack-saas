    ...
    <section className="space-y-6">
      ...
      <article className="rounded-3xl border border-white/10 bg-slate-900/40 p-6 backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/tenants" className="text-sm text-indigo-300 hover:text-indigo-200">View all</Link>
        </div>
            ...
            <tbody>
              {tenants.items.map((tenant) => (
                <tr key={tenant.tenantId} className="border-b border-white/5 text-slate-200">
                  <td className="py-2">
                    <Link href={`/tenants/${tenant.tenantId}`} className="text-indigo-300 hover:text-indigo-200">{tenant.tenantName}</Link>
                  </td>
