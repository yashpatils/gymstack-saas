...
export default function InvitesPage() {
  ...
  return <section className="space-y-6"><PageHeader title="Invites" subtitle="Invite manager, staff, and clients with strict hierarchy rules." />
    {usage ? <p className="text-xs text-muted-foreground">Staff seats used: {usage.staffSeatsUsed}/{usage.maxStaffSeats}</p> : null}
    ...
    <SectionCard title="Existing invites">{invites.length === 0 ? <EmptyState title="No invites" description="Create your first invite to onboard your team." /> : <div className="space-y-2">{invites.map((invite) => <div key={invite.id} className="flex flex-col gap-3 rounded-xl border border-border bg-card/60 p-3 text-sm text-foreground sm:flex-row sm:items-center sm:justify-between"><div className="break-words text-sm text-foreground">{invite.role} · {invite.email ?? "no email"} · <span className="text-muted-foreground">{invite.status}</span></div><button className="button secondary button-sm" onClick={async () => { await revokeTenantInvite(invite.id); setInvites(await listTenantInvites()); }}>Revoke</button></div>)}</div>}</SectionCard>
