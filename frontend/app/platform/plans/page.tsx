...
export default function PlatformPlansPage() {
  ...
  const loadPlans = async () => {
    ...
    try {
      setLoading(true);
      setPlans(await listLocationPlans(gymId));
    } catch (error) {
  ...
  const onCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setSaving(true);
      await createLocationPlan(gymId, {
        name: form.name,
  ...
  const togglePlan = async (plan: MembershipPlan) => {
    ...
    try {
      setSaving(true);
      await updateLocationPlan(gymId, plan.id, { isActive: !plan.isActive });
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        subtitle="Create and manage location-scoped membership plans."
      ...
      {showCreate ? (
        <Card title="Create membership plan" description="Define plan details for the selected location.">
          <form className="grid grid-2 gap-4" onSubmit={onCreate}>
            <Input label="Plan name" name="plan-name" required value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Plan name" />
            <select className="input" value={form.interval} onChange={(event) => setForm((current) => ({ ...current, interval: event.target.value as MembershipPlanInterval }))}>
              {Object.entries(intervalLabels).map(([value, label]) => (
                <option key={value} value={value}>{label}</option>
              ))}
            </select>
            <Input label="Price in cents" name="price-cents" value={form.priceCents} onChange={(event) => setForm((current) => ({ ...current, priceCents: event.target.value }))} placeholder="Price in cents (optional)" />
            <Input label="Description" name="description" value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Description (optional)" />
            <div>
      ...
      <section className="section">
        <Card title="Plan catalog" description="Gym-scoped plans with status controls.">
          {!gymId ? <p className="text-sm text-slate-400">Select a location to manage plans.</p> : null}
          {gymId && loading ? <p>Loading plans…</p> : null}
          {gymId && !loading && plans.length === 0 ? <EmptyState title="No plans yet" description="Create your first plan to start assigning memberships." icon={<span>◇</span>} /> : null}
          {gymId && !loading && plans.length > 0 ? (
            <Table
