...
export default async function AdminTenantsPage({ searchParams }: { searchParams?: AppSearchParams }) {
  ...
  return (
    <section className="space-y-6">
      ...
      <form className="grid gap-2 rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl md:grid-cols-[1fr_200px_auto]">
        <input name="query" defaultValue={query} className="rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white" placeholder="Search tenant" />
        <select name="status" defaultValue={status} className="rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white">
        ...
        <button type="submit" className="rounded-xl bg-indigo-500 px-4 py-2 text-sm font-medium text-white">Apply</button>
      </form>
